...
func testTodoToManyOutputAchievements(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := TodoSlice{&a}
	if err = a.L.LoadOutputAchievements(ctx, tx, false, (*[]*Todo)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testTodoToManyAddOpOutputAchievements(t *testing.T) {
	var err error
...
func testTodoToManySetOpOutputAchievements(t *testing.T) {
	var err error
	...
	err = a.SetOutputAchievements(ctx, tx, false, &b, &c)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.SetOutputAchievements(ctx, tx, true, &d, &e)
	if err != nil {
		t.Fatal(err)
	}
	...
	if a.R.OutputAchievements[0] != &d {
		t.Error("relationship struct slice not set to correct value")
	}
	if a.R.OutputAchievements[1] != &e {
		t.Error("relationship struct slice not set to correct value")
	}
}

func testTodoToManyRemoveOpOutputAchievements(t *testing.T) {
	var err error
	...
	err = a.AddOutputAchievements(ctx, tx, true, foreigners...)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.RemoveOutputAchievements(ctx, tx, foreigners[:2]...)
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(a.R.OutputAchievements) != 2 {
		t.Error("should have preserved two relationships")
	}

	// Removal doesn't do a stable deletion for performance so we have to flip the order
	if a.R.OutputAchievements[1] != &d {
		t.Error("relationship to d should have been preserved")
	}
	if a.R.OutputAchievements[0] != &e {
...
func testTodoToManyAddOpTodoDetails(t *testing.T) {
	...
	for i, x := range foreignersSplitByInsertion {
		...
		if a.TodoID != first.TodoID {
			t.Error("foreign key was wrong value", a.TodoID, first.TodoID)
		}
		if a.TodoID != second.TodoID {
			t.Error("foreign key was wrong value", a.TodoID, second.TodoID)
		}
