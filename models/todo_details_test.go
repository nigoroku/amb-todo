...
func testTodoDetailToOneTodoUsingTodo(t *testing.T) {
	...
	seed := randomize.NewSeed()
	if err := randomize.Struct(seed, &local, todoDetailDBTypes, false, todoDetailColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize TodoDetail struct: %s", err)
	}
	...
	if check.TodoID != foreign.TodoID {
