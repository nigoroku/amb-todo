...
func testTagToManyOutputAchievementTags(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := TagSlice{&a}
	if err = a.L.LoadOutputAchievementTags(ctx, tx, false, (*[]*Tag)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testTagToManyAddOpOutputAchievementTags(t *testing.T) {
	var err error
...
var (
	tagDBTypes = map[string]string{`TagID`: `int`, `Name`: `varchar`, `ColorCode`: `varchar`, `CreatedBy`: `int`, `CreatedAt`: `timestamp`, `ModifiedBy`: `int`, `ModifiedAt`: `timestamp`}
	_          = bytes.MinRead
)
