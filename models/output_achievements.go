...
var OutputAchievementWhere = struct {
	...
	Title:               whereHelperstring{field: "`output_achievements`.`title`"},
	TodoID:              whereHelpernull_Int{field: "`output_achievements`.`todo_id`"},
...
var (
	outputAchievementAllColumns            = []string{"output_achievement_id", "title", "todo_id", "reference_url", "summary", "created_by", "created_at", "modified_by", "modified_at"}
	outputAchievementColumnsWithoutDefault = []string{"title", "todo_id", "reference_url", "summary", "created_by", "created_at", "modified_by", "modified_at"}
...
func (o *OutputAchievement) Todo(mods ...qm.QueryMod) todoQuery {
...
func (outputAchievementL) LoadTodo(ctx context.Context, e boil.ContextExecutor, singular bool, maybeOutputAchievement interface{}, mods queries.Applicator) error {
...
// SetTodo of the outputAchievement to the related item.
// Sets o.R.Todo to related.
// Adds o to related.R.OutputAchievements.
func (o *OutputAchievement) SetTodo(ctx context.Context, exec boil.ContextExecutor, insert bool, related *Todo) error {
	var err error
	if insert {
	...
	values := []interface{}{related.TodoID, o.OutputAchievementID}
...
func (o *OutputAchievement) RemoveTodo(ctx context.Context, exec boil.ContextExecutor, related *Todo) error {
	var err error
	...
	for i, ri := range related.R.OutputAchievements {
		if queries.Equal(o.TodoID, ri.TodoID) {
			continue
		}

		ln := len(related.R.OutputAchievements)
		if ln > 1 && i < ln-1 {
			related.R.OutputAchievements[i] = related.R.OutputAchievements[ln-1]
		}
		related.R.OutputAchievements = related.R.OutputAchievements[:ln-1]
		break
	}
	return nil
}
