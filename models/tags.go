...
var TagWhere = struct {
	...
	ColorCode:  whereHelpernull_String{field: "`tags`.`color_code`"},
	CreatedBy:  whereHelperint{field: "`tags`.`created_by`"},
	CreatedAt:  whereHelpertime_Time{field: "`tags`.`created_at`"},
	ModifiedBy: whereHelpernull_Int{field: "`tags`.`modified_by`"},
	ModifiedAt: whereHelpernull_Time{field: "`tags`.`modified_at`"},
}
...
var (
	tagAllColumns            = []string{"tag_id", "name", "color_code", "created_by", "created_at", "modified_by", "modified_at"}
	tagColumnsWithoutDefault = []string{"name", "color_code", "created_by", "created_at", "modified_by", "modified_at"}
...
func (o *Tag) OutputAchievementTags(mods ...qm.QueryMod) outputAchievementTagQuery {
	...
	queryMods = append(queryMods,
		qm.Where("`output_achievement_tags`.`tag_id`=?", o.TagID),
	)
...
func (tagL) LoadOutputAchievementTags(ctx context.Context, e boil.ContextExecutor, singular bool, maybeTag interface{}, mods queries.Applicator) error {
	var slice []*Tag
	var object *Tag
	...
	query := NewQuery(qm.From(`output_achievement_tags`), qm.WhereIn(`output_achievement_tags.tag_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *Tag) AddOutputAchievementTags(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*OutputAchievementTag) error {
	var err error
	for _, rel := range related {
		if insert {
			...
			values := []interface{}{o.TagID, rel.OutputAchievementTagID}
...
func FindTag(ctx context.Context, exec boil.ContextExecutor, tagID int, selectCols ...string) (*Tag, error) {
	...
	query := fmt.Sprintf(
		"select %s from `tags` where `tag_id`=?", sel,
	)
...
func TagExists(ctx context.Context, exec boil.ContextExecutor, tagID int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from `tags` where `tag_id`=? limit 1)"
