...
func (t Todo) String() string {
	return fmt.Sprintf("[%d, %d, %d, %d]", t.TodoID, t.CreatedBy, t.CreatedAt, t.UserID)
}
...
func (o *Todo) OutputAchievements(mods ...qm.QueryMod) outputAchievementQuery {
	...
	queryMods = append(queryMods,
		qm.Where("`output_achievements`.`todo_id`=?", o.TodoID),
	)
...
func (todoL) LoadOutputAchievements(ctx context.Context, e boil.ContextExecutor, singular bool, maybeTodo interface{}, mods queries.Applicator) error {
	var slice []*Todo
	var object *Todo
	...
	query := NewQuery(qm.From(`output_achievements`), qm.WhereIn(`output_achievements.todo_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *Todo) AddOutputAchievements(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*OutputAchievement) error {
	var err error
	for _, rel := range related {
		if insert {
			...
			values := []interface{}{o.TodoID, rel.OutputAchievementID}
...
// SetOutputAchievements removes all previously related items of the
// todo replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.Todo's OutputAchievements accordingly.
// Replaces o.R.OutputAchievements with related.
// Sets related.R.Todo's OutputAchievements accordingly.
func (o *Todo) SetOutputAchievements(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*OutputAchievement) error {
	query := "update `output_achievements` set `todo_id` = null where `todo_id` = ?"
	...
	if o.R != nil {
		for _, rel := range o.R.OutputAchievements {
		...
		o.R.OutputAchievements = nil
	}
	return o.AddOutputAchievements(ctx, exec, insert, related...)
}

// RemoveOutputAchievements relationships from objects passed in.
// Removes related items from R.OutputAchievements (uses pointer comparison, removal does not keep order)
// Sets related.R.Todo.
func (o *Todo) RemoveOutputAchievements(ctx context.Context, exec boil.ContextExecutor, related ...*OutputAchievement) error {
	var err error
	for _, rel := range related {
		queries.SetScanner(&rel.TodoID, nil)
		if rel.R != nil {
	...
	for _, rel := range related {
		for i, ri := range o.R.OutputAchievements {
			if rel != ri {
				continue
			}

			ln := len(o.R.OutputAchievements)
			if ln > 1 && i < ln-1 {
				o.R.OutputAchievements[i] = o.R.OutputAchievements[ln-1]
			}
			o.R.OutputAchievements = o.R.OutputAchievements[:ln-1]
			break
		}
	}
