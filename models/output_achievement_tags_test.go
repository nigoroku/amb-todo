...
func testOutputAchievementTagToOneTagUsingTag(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadTag(ctx, tx, false, (*[]*OutputAchievementTag)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.Tag == nil {
...
func testOutputAchievementTagToOneSetOpTagUsingTag(t *testing.T) {
	var err error
...
var (
	outputAchievementTagDBTypes = map[string]string{`OutputAchievementTagID`: `int`, `OutputAchievementID`: `int`, `TagID`: `int`, `CreatedBy`: `int`, `CreatedAt`: `timestamp`, `ModifiedBy`: `int`, `ModifiedAt`: `timestamp`}
	_                           = bytes.MinRead
)
